...
def load_data(is_train, test_img):
    if is_train:
        ...
        #print("using only 10 data items")
        #data = data[0:30]
    else:
        if test_img != "":
...
def make_sub_data(data, padding, config):
    ...
    #file_end_index = []
    #file_end_index.append(0)

    for i in range(len(data)):

        #if i%100 == 0:
        #print("make_sub_data:", i)

        file_paths.append(data[i])
        ...
        #file_end_index.append(len(sub_input_sequence))
        
    # NOTE: The nx, ny can be ignore in train
    #print("nx, ny", nx, ny)
    #print(len(sub_input_sequence), len(sub_label_sequence))
    return sub_input_sequence, sub_label_sequence, nx, ny, original_shape, file_paths #, file_end_index
...
def input_setup(config):
    ...
    sub_input_sequence, sub_label_sequence, nx, ny, original_shape, file_paths = make_sub_data(data, padding, config)

    # Make list to numpy array. With this transform
    arrinput = np.asarray(sub_input_sequence) # [?, 33, 33, 3]
    arrlabel = np.asarray(sub_label_sequence) # [?, 21, 21, 3]

    print("invoking make_data_hf")
    make_data_hf(arrinput, arrlabel, config)
    print("make_data_hf end")

    return nx, ny, original_shape, file_paths
...
def crop_center(img, cropx, cropy):
    ...
    subb = 0
    return img[y-cropy-subb : y-subb, x-cropx-subb : x-subb, :]
